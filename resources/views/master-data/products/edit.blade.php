...
<div class="flex justify-between items-center mb-6">
    <div>
        ...
        <p class="text-gray-600">Update product: {{ $product->name }}</p>
    </div>
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200">
    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" id="product-form" x-data="productForm()">
        @csrf
        @method('PUT')
        
        <div class="p-6 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Product Information</h3>
            <p class="text-sm text-gray-600 mt-1">Update the product details</p>
        </div>
            ...
            <div>
                ...
                @if($product->photo && Storage::disk('public')->exists($product->photo))
                    <div class="mb-4">
                        ...
                        <img src="{{ Storage::url($product->photo) }}" alt="{{ $product->name }}" class="h-32 w-32 object-cover rounded-lg border border-gray-200">
                    </div>
                @endif
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gray-400 transition-colors duration-200 @error('photo') border-red-300 @enderror">
                    <div class="space-y-1 text-center" x-show="!photoPreview">
                        ...
                        <p class="text-xs text-gray-500">PNG, JPG, GIF up to 2MB (leave empty to keep current photo)</p>
                    </div>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">
            ...
            <button type="submit" 
                    ...
                    x-text="isSubmitting ? 'Updating...' : 'Update Product'">
            </button>
        </div>
    </form>
</div>
