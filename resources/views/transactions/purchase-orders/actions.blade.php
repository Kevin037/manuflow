<div class="flex justify-end gap-x-2">
    @php
        $waLink = null;
        try {
            $po = $purchaseOrder->loadMissing(['supplier','purchaseOrderDetails.material']);
            if ($po->supplier && !empty($po->supplier->phone)) {
                $lines = [];
                $lines[] = 'Purchase Order ' . ($po->no ?? '#');
                $lines[] = 'Date: ' . $po->dt->format('Y-m-d');
                $lines[] = 'Supplier: ' . $po->supplier->name;
                $lines[] = '';
                $lines[] = 'Items:';
                foreach ($po->purchaseOrderDetails as $d) {
                ...
                $lines[] = '';
                $lines[] = 'Total: Rp ' . number_format((float)$po->total, 0, ',', '.');
                $lines[] = '';
                $lines[] = 'Terima kasih.';

                $message = implode("\n", $lines);
                $phone = preg_replace('/\D+/', '', $po->supplier->phone);
                if (!empty($phone)) {
    ...
    @if($purchaseOrder->canBeModified())
        ...
        <button class="complete-btn inline-flex items-center gap-x-1.5 rounded-lg bg-green-50 px-2.5 py-1.5 text-xs font-medium text-green-600 hover:bg-green-100 transition-colors duration-200" 
        ...
        <button class="delete-btn inline-flex items-center gap-x-1.5 rounded-lg bg-red-50 px-2.5 py-1.5 text-xs font-medium text-red-600 hover:bg-red-100 transition-colors duration-200" 
